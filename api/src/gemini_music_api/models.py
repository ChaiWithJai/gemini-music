...
class StageScoreProjection(Base):
    ...
    passes_golden: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    confidence: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    scorer_source: Mapped[str] = mapped_column(String(20), nullable=False, default="deterministic", index=True)
    scorer_model: Mapped[str | None] = mapped_column(String(80), nullable=True)
    scorer_confidence: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    scorer_evidence_json: Mapped[dict] = mapped_column(JSON, default=dict, nullable=False)
